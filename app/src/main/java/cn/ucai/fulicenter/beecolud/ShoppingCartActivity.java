 ...
 */
package cn.ucai.fulicenter.beecolud;
...
import cn.ucai.fulicenter.beecolud.util.BillUtils;
import cn.ucai.fulicenter.beecolud.util.DisplayUtils;
import cn.ucai.fulicenter.utils.Json2Hashmap;
...
public class ShoppingCartActivity extends Activity {
    ...
    TextView mtvOrderBeneficiary;
    TextView mtvOrderAmount;
    TextView mtvOrderAmountType;
    TextView mtvOrderMerchandiseName;
    TextView mtvOrderId;

    private String billTitle;// 商品描述, 32个字节内, 汉字以2个字节计
    private int billTotalFee;// 支付金额，以分为单位，必须是正整数
    private String billNum;// 商户自定义订单号，PayPal不需要该参数
    private HashMap<String,String> optional;// 为扩展参数，可以传入任意数量的key/value对来补充对业务逻辑
    ...
    private void initView(){
        DisplayUtils.initBackWithTitle(this,"支付");
        mtvOrderBeneficiary = (TextView) this.findViewById(R.id.tv_order_beneficiary);
        mtvOrderAmount = (TextView) this.findViewById(R.id.tv_order_amount);
        mtvOrderAmountType = (TextView) this.findViewById(R.id.tv_order_amount_type);
        mtvOrderMerchandiseName = (TextView) this.findViewById(R.id.tv_order_merchandise_name);
        mtvOrderId = (TextView) this.findViewById(R.id.tv_order_id);
    ...
    private void initData(){
        ...
        String initInfo = BCPay.initWechatPay(ShoppingCartActivity.this, I.WEIXIN_APP_ID);
        if (initInfo != null) {
        ...
        BCPay.initPayPal(I.PAYPAL_CLIENT_ID, I.PAYPAL_SECRET,
        ...
        try {
            Map<String,Object> mpBill = Json2Hashmap.json2map(bill.toString());
            billTitle = BillUtils.getOrderBeneficiary(mpBill.get("merchant").toString());
            mtvOrderBeneficiary.setText(billTitle);
            billTotalFee = Integer.parseInt(mpBill.get("amount").toString());
            mtvOrderAmount.setText(""+BillUtils.getBillTotalFee(mpBill.get("amount").toString()));
//            mtvOrderAmountType.setText(BillUtils.getAmountType(mpBill.get("amount_type").toString()));
            mtvOrderMerchandiseName.setText(mpBill.get("extras.subject").toString());
            billNum = mpBill.get("order_no").toString();
            mtvOrderId.setText(billNum);
//            optional = mpBill;
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                switch (position) {
                    ...
                    case 2: //银联支付
                        ...
                        //支付金额，以分为单位，必须是正整数
                        payParam.billTotalFee = billTotalFee;
                    ...
                    case 4: //通过PayPal支付
                        ...
                        BCPay.getInstance(ShoppingCartActivity.this).reqPayPalPaymentAsync(
                                ...
                                billTotalFee,                      //bill amount(use cents)
                                I.CURRENCY_TYPE_USD,                  //bill currency
